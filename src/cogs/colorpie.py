...
class Colorpie(commands.Cog):
    ...
    def get_colorpie_link(self):
        ...
        url = "https://magic.wizards.com/en/section-articles-see-more-ajax?l=en&f=13541&search-result-theme=&limit=6&fromDate=&toDate=&sort=DESC&word=mechanical+color+pie+{}&offset=0".format(
            year)
        try:
            ...
            if len(data['data']) > 0:
                for el in data['data']:
                    if colorpie_url is None:
                        colorpie_url = re.search(
                            r'(\/en\/articles\/archive\/making-magic\/mechanical-color-pie-[0-9\-]*)', el)
                        if not colorpie_url is None:
                            colorpie_title = re.search(
                                r'(<h3>(.*?)<\/h3>)', el)
        except HTTPError as e:
            response = e.read()
        if not colorpie_url is None:
        ...
        if not search_str.strip() == "":
            ...
            if search_arr[0] in self.COLORS or search_arr[0] in self.COLORS.values():
                ...
                if len(colorpie) <= 1900:
                    ...
                    filename = "colorpie-{}{}.txt".format(
                        search_color, txt_number)
    ...
    async def colorpie_link(self, ctx):
            ...
            if len(data['data']) > 0:
                for el in data['data']:
                    ...
                    if colorpie_changes_url is None:
                        colorpie_changes_url = re.search(
                            r'(\/en\/articles\/archive\/making-magic\/mechanical-color-pie-[0-9]{4}-[a-z]*-[0-9\-]*)', el)
                        if not colorpie_changes_url is None:
                            colorpie_changes_title = re.search(
                                r'(<h3>(.*?)<\/h3>)', el)
        except HTTPError as e:
            response = e.read()
        message = ""
        if not colorpie_url is None:
            message += "**{}**: <https://magic.wizards.com{}>\n".format(
                colorpie_title[2], colorpie_url[0])
        if not colorpie_changes_url is None:
            message += "**{}**: <https://magic.wizards.com{}>\n".format(
                colorpie_changes_title[2], colorpie_changes_url[0])
